...
import { SpinnerIcon, XmlIcon, HtmlIcon, CodeBracketIcon, CheckIcon, CopyIcon, UploadIcon, TreeIcon } from './icons';
import { convertCodeToXml, validateCodeSyntax, correctCodeSyntax } from '../services/geminiService';
import { CodeEditor } from './CodeEditor';
import { Tooltip } from './Tooltip';
import { convertJsonToXml } from '../utils/reverseParser';
...
const defaultTabOrder: InputLanguage[] = ['json', 'html'];

export const CodeToXmlConverter: React.FC = () => {
  const [inputCodes, setInputCodes] = useState<InputCodeMap>(() => {
    try {
     const stored = localStorage.getItem('codeToXmlInputs');
  ...
  const [tabOrder, setTabOrder] = useState<InputLanguage[]>(() => {
    try {
        const storedOrder = localStorage.getItem('xmlConverterTabOrder');
        if (storedOrder) {
  ...
  const handleDrop = () => {
    ...
    localStorage.setItem('xmlConverterTabOrder', JSON.stringify(newTabOrder));
  };
  ...
  const handleInputChange = (value: string) => {
      ...
      debounceTimeoutRef.current = window.setTimeout(() => {
        localStorage.setItem('codeToXmlInputs', JSON.stringify(newInputs));
      }, 500);
  };
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (activeTab === 'json') {
        ...
        resultXml = convertJsonToXml(jsonObj);
      } else { // HTML
        resultXml = await convertCodeToXml(currentInput, activeTab);
      }
      
      setOutputXml(resultXml);
      setIsConverted(true);
      setActiveOutputTab('code');
    } catch (err: any) {
      const isLocal = activeTab === 'json';
      let errorMessage = err.message || `Failed to convert ${activeTab} to XML.`;
      if (isLocal) {
          errorMessage = `Conversion from ${activeTab} to XML failed. This usually means the input data has a formatting error.\n\nDetails: ${err.message}`;
      }
  ...
  const convertTooltip = hasError
    ...
    : "Convert the validated code to XML.";

  return (
    <>
      <SEO
        title="Code to XML Converter | AI JSON Tools"
        description="Convert JSON and HTML code to XML instantly using AI-powered tools. Validate, correct, and visualize XML output with ease."
        keywords="code to xml converter, AI XML Tools, convert code, JSON to XML, HTML to XML"
        canonical="https://yourdomain.com/code-to-xml"
        ogImage="https://yourdomain.com/images/code-to-xml.jpg"
