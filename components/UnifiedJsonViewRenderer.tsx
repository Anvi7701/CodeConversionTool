...
const TreeNode: React.FC<TreeNodeProps> = ({ keyName, value, level, isLast: _isLast, segments, rootData, onRootUpdate, expandAll, collapseAll, selectedPath, onSelectPath }) => {
  ...
  const renderValue=()=> isEditingValue? (<input ref={valueInputRef} className="px-1 py-0.5 text-sm border rounded bg-white dark:bg-slate-900" value={editedValue} onChange={e=>setEditedValue(e.target.value)} onBlur={saveEditValue} onKeyDown={e=>{ if(e.key==='Enter') saveEditValue(); else if(e.key==='Escape') cancelEditValue(); }} />): value===null? <span onClick={onRootUpdate ? startEditValue : undefined} className={onRootUpdate ? "cursor-text text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 px-1.5 py-0.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-200 inline-block hover:scale-105" : "text-slate-400 px-1.5 py-0.5"} title={onRootUpdate ? "Click to edit" : undefined}>null</span>: typeof value==='boolean'? (<div className="inline-flex items-center gap-2 px-1.5 py-0.5 rounded bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/30 dark:to-pink-900/30"><input type="checkbox" checked={value} onChange={onRootUpdate ? handleBooleanToggle : undefined} disabled={!onRootUpdate} className={onRootUpdate ? "w-4 h-4 cursor-pointer accent-purple-600 dark:accent-purple-400 rounded" : "w-4 h-4 accent-purple-600 dark:accent-purple-400 rounded opacity-60 cursor-not-allowed"} title={value ? 'Checked (true)' : 'Unchecked (false)'} /><span className="text-purple-600 dark:text-purple-400 font-semibold text-sm select-none">{String(value)}</span></div>): typeof value==='number'? <span onClick={onRootUpdate ? startEditValue : undefined} className={onRootUpdate ? "cursor-text text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-semibold px-1.5 py-0.5 rounded hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 dark:hover:from-blue-900/30 dark:hover:to-cyan-900/30 transition-all duration-200 inline-block hover:scale-105 hover:shadow-sm" : "text-blue-600 dark:text-blue-400 font-semibold px-1.5 py-0.5"} title={onRootUpdate ? "Click to edit" : undefined}>{value}</span>: typeof value==='string'? <span onClick={onRootUpdate ? startEditValue : undefined} className={onRootUpdate ? "cursor-text text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 px-1.5 py-0.5 rounded hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 dark:hover:from-green-900/30 dark:hover:to-emerald-900/30 transition-all duration-200 inline-block hover:scale-105 hover:shadow-sm" : "text-green-600 dark:text-green-400 px-1.5 py-0.5"} title={onRootUpdate ? "Click to edit" : undefined}>"{value}"</span>: null;
  ...
  return (
    <div className={`font-mono text-sm ${level===0?'':'ml-4'}`}>
      <div className={`flex items-start gap-1 hover:bg-slate-100 dark:hover:bg-slate-800 py-0.5 pl-1 pr-2 rounded group relative transition-all duration-200 hover:shadow-sm ${getValueTypeStyle()} ${isHighlighted ? 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/30' : ''}`} onDragOver={handleDragOver} onDrop={handleDrop} onDragLeave={handleDragLeave} onClick={handleNodeClick}>
        ...
        {level>0 && onRootUpdate && (containerType==='array' || containerType==='object') && (
        ...
        <div className="flex-1 break-all">
          {isEditingKey? (
            ...
            <span onClick={onRootUpdate ? startEditKey : undefined} title={!onRootUpdate ? '' : (Array.isArray(getDataAtPath(rootData,segments.slice(0,-1)))|| level===0? '': 'Click to rename key')} className={`text-teal-700 dark:text-teal-400 font-bold ${onRootUpdate ? 'cursor-text' : ''} select-text bg-teal-50/50 dark:bg-teal-900/20 px-1.5 py-0.5 rounded ${onRootUpdate ? 'hover:bg-teal-100 dark:hover:bg-teal-900/40' : ''} transition-all duration-200 inline-block`}>
              {keyName}
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-900">
      ...
      <div className="flex-1 overflow-auto p-4" ref={scrollRef}>
        <TreeNode key="root" keyName="root" value={treeData} level={0} isLast={true} segments={[]} rootData={treeData} onRootUpdate={onEdit ? handleTreeUpdate : undefined} expandAll={expandAll} collapseAll={collapseAll} selectedPath={selectedPath} onSelectPath={handleSelectPath} />
      </div>
    </div>
  );
};
