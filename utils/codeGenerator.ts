...
const toJava = (obj: any): string => {
  const toJavaValue = (value: any, currentIndent: number): string => {
    const indentStr = ' '.repeat(currentIndent);
    const innerIndent = ' '.repeat(currentIndent + 4);
    ...
    if (Array.isArray(value)) {
      ...
      const items = value.map(item => toJavaValue(item, currentIndent + 4)).join(`,\n${innerIndent}`);
      return `new ArrayList<>(Arrays.asList(\n${innerIndent}${items}\n${indentStr}))`;
    }

    if (typeof value === 'object') {
        if (Object.keys(value).length === 0) return 'new HashMap<>()';
        const entries = Object.entries(value).map(([k, v]) => 
            `${innerIndent}subMap.put("${k}", ${toJavaValue(v, currentIndent + 4)});`
        ).join('\n');

        return `(() -> {\n${indentStr}    Map<String, Object> subMap = new HashMap<>();\n${entries}\n${indentStr}    return subMap;\n${indentStr}})()`;
    }
    return 'null';
  }

  const mainMap = Object.entries(obj).map(([key, value]) => 
    `        data.put("${key}", ${toJavaValue(value, 8)});`
  ).join('\n');
...
const toCSharp = (json: any): string => {
    const jsonString = JSON.stringify(json, null, 2);
    // For C# multiline strings, escape quotes by doubling them
    const csharpJsonString = jsonString.replace(/"/g, '""');
